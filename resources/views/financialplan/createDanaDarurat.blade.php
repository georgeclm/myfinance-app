<div class="modal fade" id="danaDarurat" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
    aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="bg-dark  modal-content">
            ...
            <div class="modal-body">
                <form id="addDanaDarurat" method="POST" action="{{ route('financialplans.danadarurat') }}">
                    @csrf
                    <div class="mb-3 hide-inputbtns input-group">
                        ...
                        <input data-number-stepfactor="100" type="number" name="jumlah" value="{{ old('jumlah') }}"
                            required placeholder="Pengeluaran Bulanan"
                    ...
                    <div class="form-group">
                        <select class="form-control form-control-user form-block @error('status') is-invalid @enderror"
                            name="status" value={{ old('status') }} style="padding: 0.5rem !important" required
                            aria-describedby="emailHelp">
                            <option value="" selected disabled hidden>Status Pernikahan</option>
                            <option value="1">Lajang</option>
                            <option value="2">Menikah</option>
                            <option value="3">Menikah dan memiliki anak</option>
                        </select>
                        @error('status')
                            <span class="invalid-feedback" role="alert">
                                <strong>{{ $message }}</strong>
